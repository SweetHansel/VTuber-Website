...
async function getTrackedSocialsFromCMS(): Promise<Social[]> {
  try {
    ...
    console.warn(
      "Could not fetch tracked socials from CMS, using fallback:",
      error,
    );
    return [];
  }
...
export async function GET() {
  try {
    ...
    const youtubeSocials = trackedSocials.filter(
    ...
    if (override?.isLive) {
      ...
      const platform: "twitch" | "youtube" = url.includes("twitch")
        ? "twitch"
        : "youtube";
  ...
  return (liveStreams || []).map(
    (stream: Record<string, unknown>, index: number) => {
      const social = socials.find(
        (s) =>
          extractChannelId(s.url, "twitch").toLowerCase() ===
